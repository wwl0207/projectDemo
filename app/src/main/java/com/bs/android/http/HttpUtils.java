...
import com.bs.android.http.md5.MD5Utils;
...
//                        String filepath=Environment.getExternalStoragePublicDirectory(DIRECTORY_DOWNLOADS).getPath()+"/"+filename;
                        String filepath= CommentUtil.getAppRootPath(App.instance).getPath()+"/shopping/"+filename;
    ...
    public static void POST(final String url, Map<String,String> param, final boolean isHandleError, final Class<?> cls, final Callback callback) {
        ...
        param.put("equipment_num", Settings.Secure.getString(App.instance.getContentResolver(), Settings.Secure.ANDROID_ID) );
...
//                                new org.json.JSONObject(json);
        ...
        for (String mapkey:filePathMaps.keySet()){
            ...
            for (int i=0;i<filesPath.size();i++){
                ...
                if (!TextUtils.isEmpty(filePath)){
                     ...
                     * 文件路径为空
                     */
                    File file = new File(App.instance.getFilesDir().getPath()+"emptyfilelyk");
                    if (file.exists()){
                        Log.i("logger",file.getPath());
                        builder.addFormDataPart(mapkey+"["+i+"]",file.getName(),RequestBody.create(MediaType.parse("image/*"), file));
                    }else {
     ...
     * @param json
     */
    public static void handleOtherStatus(String json) {

        String msg = JsonUtils.getSinglePara(json, "desc");
        String code = JsonUtils.getSinglePara(json, "code");
        if (!code.equals("500")) {
    ...
    private static void writeFile(ResponseBody responseBody, String filePath, Callback callback) {
        ...
        try {
            ...
            if (callback!=null){
                callback.onSucceed(file.getPath(),"200","");
            }
            inputString.close();
            fos.close();
        } catch (Exception e) {
